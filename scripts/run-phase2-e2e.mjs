...
async function stop(child) { child.kill(); if (child.exitCode === null) await Promise.race([once(child, "exit"), new Promise((done) => setTimeout(done, 2000))]); }
let code = 1;
try {
...
} finally { await Promise.all([stop(app), stop(staticServer)]); }
