...
const environment = {
  ...process.env, NEXT_PUBLIC_SUPABASE_URL: status.API_URL, NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: status.PUBLISHABLE_KEY,
  SUPABASE_URL: status.API_URL, SUPABASE_PUBLISHABLE_KEY: status.PUBLISHABLE_KEY, SUPABASE_SECRET_KEY: status.SECRET_KEY,
  APP_BASE_URL: "http://127.0.0.1:3000", LEGACY_GAME_URL: "http://127.0.0.1:4173/docs/index.html",
  BILLING_ENABLED: "true", BILLING_ENVIRONMENT: "test", BILLING_PROVIDER: "fixture", STRIPE_MODE: "test", STRIPE_API_VERSION: "2026-07-29.dahlia",
  ...
  STRIPE_PRODUCT_TEACHER_PRO: "prod_fixture123", STRIPE_PRICE_TEACHER_PRO_MONTHLY: "price_monthly123", STRIPE_PRICE_TEACHER_PRO_ANNUAL: "price_annual123",
...
async function waitFor(url) { const end = Date.now() + 45_000; while (Date.now() < end) { try { if ((await fetch(url)).ok) return; } catch { /* server is still starting */ } await new Promise((done) => setTimeout(done, 200)); } throw new Error("Phase 2 server unavailable"); }
async function stop(child, isNext = false) { if (isNext) return stopVerificationNextProcess(child); child.kill(); if (child.exitCode === null) await Promise.race([once(child, "exit"), new Promise((done) => setTimeout(done, 2000))]); }
let code = 1;
try {
  await Promise.all([waitFor("http://127.0.0.1:3000/pricing"), waitFor("http://127.0.0.1:4173/docs/index.html")]);
  const test = spawn(process.execPath, [resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.phase2.config.mjs"], { env: { ...environment, SUPABASE_TEST_URL: status.API_URL, SUPABASE_TEST_SECRET_KEY: status.SECRET_KEY }, stdio: "inherit" });
  [code] = await once(test, "exit");
} finally { await Promise.all([stop(app, true), stop(staticServer)]); }
