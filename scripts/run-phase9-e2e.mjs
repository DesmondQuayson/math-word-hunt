...
const environment = {
  ...
  MVH_BUILD_ID: "phase9-teacher-first-local",
...
async function waitFor(url) {
  ...
  throw new Error("Phase 9 local platform did not become ready.");
}

let exitCode = 1;
try {
  await waitFor("http://127.0.0.1:3000/");
  const tests = spawn(process.execPath, [resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.phase9.config.mjs", ...process.argv.slice(2)], {
    env: {
      ...environment,
