...
$verified = $null
try {
  foreach ($name in @('SUPABASE_ACCESS_TOKEN', 'SUPABASE_DB_PASSWORD', 'STRIPE_PUBLISHABLE_KEY', 'STRIPE_SECRET_KEY')) {
  ...
  foreach ($retired in @('RESEND_API_KEY', 'RESEND_PROVISIONING_API_KEY')) {
  ...
  $resend = Read-RequiredSecret 'Temporary Resend Full-access API key' { param($value) $value -match '^re_[A-Za-z0-9_\-]{16,}$' }
  $vault.Values | Add-Member -MemberType NoteProperty -Name RESEND_PROVISIONING_API_KEY -Value $resend -Force
  $vault | Export-Clixml -LiteralPath $pendingPath
  $verified = Import-Clixml -LiteralPath $pendingPath
  $serialized = Get-Content -LiteralPath $pendingPath -Raw
  Assert-ImportedSecret $verified.Values.RESEND_PROVISIONING_API_KEY { param($value) $value -match '^re_[A-Za-z0-9_\-]{16,}$' } $serialized
...
Write-Host 'PHASE 7D RESEND PROVISIONING KEY ACCEPTED'
