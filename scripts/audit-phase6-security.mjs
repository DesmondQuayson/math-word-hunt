...
requireText("apps/platform-web/components/pilot/pilot-status-banner.tsx", ["Adult teachers only", "No student data", "No organization labels", "No billing", "data-pilot-activation"]);
requireText("apps/platform-web/lib/pilot/copy.ts", ["Pilot inactive", "Pilot preparing", "Ready for owner decision", "Controlled pilot active", "Pilot paused", "Pilot ended"]);
requireText("apps/platform-web/app/pilot/privacy/page.tsx", ["not legal advice", "No final retention duration"]);
requireText("apps/platform-web/app/pilot/support/page.tsx", ["Contact the pilot coordinator using the channel through which pilot access was provided", "acknowledgment within one business day", "not 24/7 emergency support"]);
requireText("apps/platform-web/components/auth-email-status.tsx", ["data-auth-email-state", "getAuthEmailExperience"]);
requireText("apps/platform-web/lib/email/server.ts", ["local-capture", "transactional-configured", "transactional-verified", "If that teacher account exists"]);
requireText("apps/platform-web/app/forgot-password/page.tsx", ["Responses remain generic", "never confirms whether an email address belongs"]);
...
if (!serverPolicy.includes("MVH_PILOT_STATE") || !serverPolicy.includes("MVH_PILOT_OWNER_GO") || !serverPolicy.includes("MVH_PILOT_AUTH_EMAIL_VERIFIED") || /NEXT_PUBLIC_MVH_PILOT/.test(serverPolicy)) throw new Error("Pilot policy adapter is not server-only and fail-closed.");
...
if (!example.includes("MVH_PILOT_STATE=inactive") || !example.includes("MVH_PILOT_OWNER_GO=incomplete") || !example.includes("MVH_EMAIL_DELIVERY=local-capture") || /NEXT_PUBLIC_MVH_PILOT/.test(example)) throw new Error("Pilot environment example is not server-only and default inactive.");
