...
const environment = {
  ...
  MVH_BUILD_ID: "phase7b-local-rehearsal",
...
async function waitFor(url) {
  ...
  throw new Error("Production-platform rehearsal did not become ready.");
}

let exitCode = 1;
try {
  await waitFor("http://127.0.0.1:3000/");
  const tests = spawn(process.execPath, [resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.phase7b.config.mjs"], {
    env: {
      ...environment,
