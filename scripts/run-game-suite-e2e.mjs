...
if (status.API_URL !== "http://127.0.0.1:55321") throw new Error("Game-suite browser verification is local-only.");

execFileSync(process.execPath, [cli, "db", "reset", "--local"], { stdio: "inherit" });
...
const environment = {
  ...
  MVH_BUILD_ID: "game-suite-polish-local",
  MVH_ADMIN_ENABLED: "true",
  MVH_ADMIN_CSRF_SECRET: "game-suite-local-admin-csrf-secret",
  MVH_ADMIN_SESSION_MINUTES: "15",
  MVH_GAME_DELIVERY_SECRET: "game-suite-local-delivery-secret-value",
...
async function waitForApp() {
  ...
  while (Date.now() < deadline) {
    if (app.exitCode !== null) throw new Error("Game-suite application exited before readiness.");
    try {
  ...
  throw new Error("Game-suite application did not become ready.");
}
...
try {
  ...
  const tests = spawn(process.execPath, [
    resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.game-suite.config.mjs", ...process.argv.slice(2)
  ], {
    env: { ...environment, SUPABASE_TEST_URL: status.API_URL, SUPABASE_TEST_SECRET_KEY: status.SECRET_KEY },
    stdio: "inherit"
  });
  ...
  try {
    ...
    console.error("Failed to restore the local database after game-suite verification.", error);
    exitCode = 1;
  }
}
