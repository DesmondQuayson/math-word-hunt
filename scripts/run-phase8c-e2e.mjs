...
if (!/^http:\/\/127\.0\.0\.1:55321$/.test(status.API_URL)) throw new Error("Phase 8C browser verification is local-only.");

const admin = createClient(status.API_URL, status.SECRET_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
const migration = await admin.from("content_resources").select("id", { count: "exact", head: true });
if (migration.error) throw new Error("Apply the Phase 8B local migration before Phase 8C browser verification.");
...
const environment = {
  ...
  MVH_BUILD_ID: "phase8c-local-admin-ui",
  MVH_ADMIN_ENABLED: "true",
  MVH_ADMIN_CSRF_SECRET: "phase8c-local-browser-only-secret-value",
...
async function waitFor(url) {
  ...
  throw new Error("Phase 8C local admin interface did not become ready.");
}
...
try {
  ...
  const tests = spawn(process.execPath,
    [resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.phase8c.config.mjs"],
