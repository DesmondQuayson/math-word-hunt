...
if (status.API_URL !== "http://127.0.0.1:55321") throw new Error("CrossCalc V2 browser verification is local-only.");
...
const environment = {
  ...
  MVH_STRIPE_MODE: "test", MVH_FIXTURE_POLICY: "forbidden", MVH_BUILD_ID: "crosscalc-v2-native-local", MVH_ADMIN_ENABLED: "true",
  MVH_ADMIN_CSRF_SECRET: "crosscalc-v2-local-admin-csrf-secret", MVH_ADMIN_SESSION_MINUTES: "15", MVH_GAME_DELIVERY_SECRET: "crosscalc-v2-local-delivery-secret-value",
...
async function waitForApp() {
  ...
  while (Date.now() < deadline) {
    if (app.exitCode !== null) throw new Error("CrossCalc V2 application exited before readiness.");
    try { const response = await fetch("http://127.0.0.1:3000/"); if (response.status < 500) return; } catch { /* Next is starting. */ }
    await new Promise((done) => setTimeout(done, 200));
  }
  throw new Error("CrossCalc V2 application did not become ready.");
}

let exitCode = 1;
try {
  await waitForApp();
  const tests = spawn(process.execPath, [resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.crosscalc-v2.config.mjs", ...process.argv.slice(2)], {
  ...
  catch (error) { console.error("Failed to restore the local database after CrossCalc V2 verification.", error); exitCode = 1; }
}
