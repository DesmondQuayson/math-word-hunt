...
if (status.API_URL !== "http://127.0.0.1:55321") throw new Error("Native Number Cross browser verification is local-only.");
...
const environment = {
  ...
  MVH_BUILD_ID: "number-cross-native-local",
  MVH_ADMIN_ENABLED: "true",
  MVH_ADMIN_CSRF_SECRET: "number-cross-local-admin-csrf-secret",
  MVH_ADMIN_SESSION_MINUTES: "15",
  MVH_GAME_DELIVERY_SECRET: "number-cross-local-delivery-secret-value",
...
async function waitForApp() {
  ...
  while (Date.now() < deadline) {
    if (app.exitCode !== null) throw new Error("Native Number Cross application exited before readiness.");
    try {
  ...
  throw new Error(`Native Number Cross application did not become ready (last status: ${lastStatus}).`);
}
...
try {
  ...
  const tests = spawn(process.execPath, [
    resolve("node_modules/@playwright/test/cli.js"), "test", "--config=playwright.number-cross.config.mjs",
  ...
  try {
    ...
    console.error("Failed to restore the local database after native Number Cross verification.", error);
    exitCode = 1;
  }
}
