...
const unavailableMetrics: readonly AdminMetric[] = [
  ...
  ["downloads", "Recent downloads", "Download events are not collected yet"],
...
export async function loadAdminDashboard(): Promise<AdminDashboardSnapshot> {
  ...
  if (!client) return {
    state: "unavailable", metrics: unavailableMetrics, emailHealth: "not-configured",
    webhookHealth: "unavailable", systemHealth: "degraded", recentActions: []
  };

  const [games, drafts, homework, quizzes, subscribers, trials, failedPayments, webhookFailures, webhookEvents, actions] = await Promise.all([
  ...
  const results = [games, drafts, homework, quizzes, subscribers, trials, failedPayments, webhookFailures, webhookEvents, actions];
  if (results.some((result) => result.error)) return {
    state: "unavailable", metrics: unavailableMetrics, emailHealth: "not-configured",
    webhookHealth: "unavailable", systemHealth: "degraded", recentActions: []
  };

  const count = (value: typeof games) => value.count ?? 0;
