...
export class SupabaseConsumerEntitlementRepository {
  ...
  async getEvidence(account: ConsumerAccountRecord): Promise<MathNexaAllAccessEvidence | Record<string, never>> {
    ...
    if (!complimentary.error && typeof complimentaryExpiry === "string" &&
      ...
      return {
        ...
        entitlement: { state: "subscription-active", periodEndsAt: complimentaryExpiry }
      };
    }
    const { data, error } = await this.client
      .from("consumer_game_entitlements")
      .select("capability_key, entitlement_state, trial_started_at, trial_ends_at, current_period_ends_at, grace_ends_at")
      .eq("user_id", account.userId)
      .maybeSingle();
    if (error) return {};
    if (!data) return {
      capabilityKey: MATHNEXA_ALL_ACCESS,
      entitlement: { state: "no-entitlement", trialRedeemedAt: account.trialRedeemedAt }
    };
    if (data.capability_key !== MATHNEXA_ALL_ACCESS) return {};

    let entitlement: GameEntitlementEvidence | null = null;

    if (data.entitlement_state === "trial-pending" && account.trialRedeemedAt) {
      entitlement = { state: "trial-pending", trialRedeemedAt: account.trialRedeemedAt };
    }
    if (data.entitlement_state === "trial-active" && account.trialRedeemedAt && data.trial_started_at && data.trial_ends_at) {
      entitlement = { state: "trial-active", trialRedeemedAt: account.trialRedeemedAt, startsAt: data.trial_started_at, endsAt: data.trial_ends_at };
    }
    if (data.entitlement_state === "trial-expired" && account.trialRedeemedAt && data.trial_ends_at) {
      entitlement = { state: "trial-expired", trialRedeemedAt: account.trialRedeemedAt, endedAt: data.trial_ends_at };
    }
    if (data.entitlement_state === "subscription-active" && data.current_period_ends_at) {
      entitlement = { state: "subscription-active", periodEndsAt: data.current_period_ends_at };
    }
    if (data.entitlement_state === "subscription-past-due") {
      entitlement = { state: "subscription-past-due", periodEndsAt: data.current_period_ends_at };
    }
    ...
    if (data.entitlement_state === "subscription-canceled-through-period-end" && data.current_period_ends_at) {
      entitlement = { state: "subscription-canceled-through-period-end", periodEndsAt: data.current_period_ends_at };
    }
    if (data.entitlement_state === "subscription-expired" && data.current_period_ends_at) {
      entitlement = { state: "subscription-expired", endedAt: data.current_period_ends_at };
    }
    if (data.entitlement_state === "no-entitlement") {
      entitlement = { state: "no-entitlement", trialRedeemedAt: account.trialRedeemedAt };
    }
    return entitlement ? { capabilityKey: MATHNEXA_ALL_ACCESS, entitlement } : {};
  }
}
