...
/**
 * Classifies an IP address literal for egress safety.
 *
 * This is the address logic that `isPublicInternetAddress` has applied since
 * the overnight SSRF hardening — moved into a dependency-free module so both
 * the destination health check and the security alert sink can use it, and
 * extended to say WHY an address is refused. The classification is recorded
 * in the SSRF security event; the address itself never is.
 */
export const INTERNET_ADDRESS_CLASSES = [
  "public", "loopback", "private", "link-local", "multicast", "unspecified", "reserved", "invalid"
] as const;
export type InternetAddressClass = (typeof INTERNET_ADDRESS_CLASSES)[number];

function classifyIpv4(address: string): InternetAddressClass {
  const octets = address.split(".").map(Number);
  if (octets.length !== 4 || octets.some((part) => !Number.isInteger(part) || part < 0 || part > 255)) return "invalid";
  ...
  if ((a === 192 && b === 0) || (a === 198 && (b === 18 || b === 19))) return "reserved";
  return "public";
}
 ...
 *
 * Deciding IPv6 by string prefix is what made an earlier version wrong, in
...
export function classifyInternetAddress(address: string): InternetAddressClass {
  ...
  if (bytesAreZero(bytes, 0, 15)) return bytes[15] === 0 ? "unspecified" : bytes[15] === 1 ? "loopback" : "public";

  const embeddedV4 = () => bytes.slice(12, 16).join(".");

  // IPv4-mapped ::ffff:0:0/96 and IPv4-translated ::ffff:0:0:0/96.
  if (bytesAreZero(bytes, 0, 10) && bytes[10] === 0xff && bytes[11] === 0xff) return classifyIpv4(embeddedV4());
  // IPv4-compatible ::a.b.c.d (deprecated but still routable through some stacks).
  if (bytesAreZero(bytes, 0, 12)) return classifyIpv4(embeddedV4());
  ...
  if (bytes[0] === 0x20 && bytes[1] === 0x02) return classifyIpv4(bytes.slice(2, 6).join("."));
  ...
  if (bytes[0] === 0xfe && (bytes[1]! & 0xc0) === 0x80) return "link-local";  // fe80::/10 link-local
  if (bytes[0] === 0xff) return "multicast";                                  // ff00::/8  multicast
  if (bytes[0] === 0x01 && bytesAreZero(bytes, 1, 8)) return "reserved";      // 100::/64  discard-only
  return "public";
}

export function isPublicInternetAddress(address: string): boolean {
  return classifyInternetAddress(address) === "public";
}
