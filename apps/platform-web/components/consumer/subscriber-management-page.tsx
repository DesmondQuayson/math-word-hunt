...
export async function SubscriberManagementPage({ rollbackSafe = false }: Readonly<{ rollbackSafe?: boolean }>) {
  ...
  const subscription = repository && context.userId
    ? await repository.getAuthoritativeSubscription(context.userId).catch(() => null)
    : null;
  ...
  return <Container className="page-stack" width="compact">
    ...
    {subscription ? <ConsumerSubscriptionSummary subscription={subscription} /> : <Notice label="Subscription record" tone="information"><strong>No current subscription was found.</strong><p>No cancellation action is required unless support identifies an unresolved provider record.</p></Notice>}
