...
export async function GET(_request:Request,{params}:{params:Promise<{resourceId:string;fileId:string}>}){
  ...
  const file=await client.from("resource_files").select("bucket_id,object_path,mime_type").eq("id",fileId).eq("resource_id",resourceId).eq("resource_version_number",resource.data.published_version_number).eq("validation_state","accepted").in("file_role",["thumbnail","preview_image"]).maybeSingle();
