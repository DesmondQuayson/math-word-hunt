...
/**
 * Inline delivery of a published Quiz PDF for the in-app preview. The
 * authorization is the download route's, step for step: the server access
 * decision, the database's record_resource_download authorization for a
 * consumer principal, then a 60-second signed URL fetched here and streamed.
 * Only the disposition differs (inline instead of attachment), so a browser or
 * the preview viewer shows the bytes instead of saving them. Quiz resources
 * only; every other resource type is not found. No storage URL ever reaches
 * the browser; the response is never cached.
 */
export async function GET(_request:Request,{params}:{params:Promise<{resourceId:string}>}){
  ...
  if(resource.error||!resource.data||!resource.data.resource_type.startsWith("quiz_"))return Response.json({error:"not-found"},{status:404});
  ...
  return new Response(fetched.body,{headers:{"Content-Type":"application/pdf","Content-Disposition":`inline; filename="${file.data.normalized_filename.replace(/["\\\r\n]/g,"")}"`,"Cache-Control":"private, no-store, max-age=0","X-Content-Type-Options":"nosniff","Content-Security-Policy":"default-src 'none'; frame-ancestors 'self'","Referrer-Policy":"no-referrer"}});
}
