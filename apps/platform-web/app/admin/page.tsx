...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ csrf?: string; section?: string; upload?: string; publish?: string; package?: string; cms?:string; media?:string; account?:string; from?:string; to?:string; ops?:string; taxonomy?:string; map?:string }> }) {
  ...
  const taxonomy = ["games","homework","quizzes"].includes(allowedSection) ? await loadAdminTaxonomy() : null;
