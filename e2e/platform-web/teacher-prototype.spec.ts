...
test("demonstration records support the planned browse path without fake persistence", async ({ page }) => {
  ...
  await expect(page.getByText(/student-level tracking/i)).toBeVisible();
});
...
test("demonstration report data remains labeled and readable at 400 percent reflow", async ({ page }) => {
  ...
  const layout = await page.evaluate(() => ({
    ...
    firstCellDisplay: getComputedStyle(document.querySelector(".data-table td")!).display
  }));
  expect(layout.scrollWidth).toBeLessThanOrEqual(layout.viewportWidth);
  expect(layout.firstCellDisplay).toBe("grid");
});
